...
- **Railway** (current hosting): Root Directory must be `/server`. `railway.toml` builds with Nixpacks, runs migrations, then `node dist/src/server.js` on `0.0.0.0:$PORT` (do **not** set `PORT` yourself). Health check path is `/health`. Paste variables from `.env.production` into the service Variables tab **without quotes**. Use the Supabase JWT `anon` / `service_role` keys (the `eyJ…` keys), not only the `sb_publishable_` / `sb_secret_` keys. The storefront and admin are separate services; their `VITE_API_URL` comes from `.env.production` (a Railway variable of the same name overrides it).
...
- Production checklist: `NODE_ENV=production`, `API_BASE_URL`, `FRONTEND_URL`, `ADMIN_FRONTEND_URL` (these two are the CORS allow-list and email link bases), `DATABASE_URL` + `DATABASE_SSL=true`, `SUPABASE_URL`, `SUPABASE_ANON_KEY` (JWT anon public key), `SUPABASE_SERVICE_ROLE_KEY` (JWT service_role key — server only), `AUTH_PROVIDER=supabase`, `STORAGE_PROVIDER=supabase`, strong `JWT_SECRET`, `COOKIE_SECURE=true` (and `COOKIE_SAMESITE=none` + HTTPS if the API is on a different site than the frontends), real email provider, `PAYMENT_PROVIDERS` without `mock`.

`GET /health` → `{ status, environment, timestamp, checks: { database } }` (always 200 once the process is listening; `checks.database` is `down` when Postgres is unreachable). `GET /health/ready` returns 503 when the database is unreachable.
