...
export function QuickViewModal() {
  ...
  return (
    <Modal open={Boolean(slug)} onClose={close} title={product?.name ?? t('product.quickView.title')} hideTitle size="xl">
      {loading || (!product && !error) ? (
        ...
        <ErrorState title={t('product.quickView.unavailable')} message={error ?? t('product.quickView.notFound')} onRetry={error ? reload : undefined} />
      ) : (
        <div className="grid md:grid-cols-2">
              ...
              <div className="scrollbar-none flex gap-2 overflow-x-auto p-3">
                {product.images.map((img, i) => (
                  <button
                    ...
                    aria-label={t('product.gallery.showImage', { n: i + 1 })}
                    aria-pressed={i === imageIdx}
                    className={cn('relative aspect-square w-14 shrink-0 overflow-hidden border', i === imageIdx ? 'border-ink' : 'border-transparent opacity-70 hover:opacity-100')}
                  >
          ...
          <div className="flex flex-col gap-6 p-5 sm:p-8">
            ...
            <div className="flex gap-2 sm:gap-3">
              ...
              <Button variant="primary" fullWidth className="min-w-0 flex-1 px-3" onClick={handleAdd} loading={sel.adding} disabled={sel.soldOut || sel.variantSoldOut}>
                {sel.soldOut ? t('common.states.outOfStock') : t('common.actions.addToBag')}
              </Button>
              <WishlistButton product={product} variant="outline" className="h-12 w-12 shrink-0" />
            </div>
            <Link to={productPath(product.slug)} onClick={close} className="group inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-ink">
              <span className="link-underline">{t('product.quickView.viewDetails')}</span>
