...
export function ProductGallery({ images, activeIndex = 0, badges }: ProductGalleryProps) {
  ...
  useEffect(() => {
    ...
    if (el) el.scrollTo({ left: el.clientWidth * activeIndex, behavior: 'smooth' });
  }, [activeIndex]);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative lg:hidden">
        <div ref={strip} onScroll={onStripScroll} className="scrollbar-none flex snap-x snap-mandatory overflow-x-auto" aria-label="Product images">
          {images.map((img, i) => (
      ...
      <div className="hidden gap-4 lg:grid lg:grid-cols-[76px_1fr] xl:grid-cols-[88px_1fr]">
        <div className="flex flex-col gap-3" role="tablist" aria-label="Product image thumbnails" aria-orientation="vertical">
          {images.map((img, i) => (
            <button
              key={img.url + i}
              type="button"
        ...
        >
          ...
          />
          <span className="pointer-events-none absolute bottom-4 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-ink opacity-100 transition-opacity group-hover:opacity-0" aria-hidden>
            <ZoomIn className="h-4 w-4" />
          </span>
        </div>
      </div>

      {badges && <div className="pointer-events-none absolute left-4 top-4 flex flex-col items-start gap-1.5 lg:left-[92px] xl:left-[104px]">{badges}</div>}
    </div>
  );
}
