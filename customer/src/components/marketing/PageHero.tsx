...
export function PageHero({ eyebrow, title, description, image, crumbs, meta, size = 'md' }: PageHeroProps) {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-ink text-white">
      ...
      <div className="absolute inset-0 -z-10 bg-gradient-to-r from-ink/90 via-ink/60 to-ink/10" aria-hidden />
      <div className={cn('container-site flex flex-col justify-end pb-10 pt-8 sm:pb-14', size === 'lg' ? 'min-h-[420px] sm:min-h-[520px]' : 'min-h-[300px] sm:min-h-[380px]')}>
        ...
        <h1 className="mt-3 font-display text-5xl font-extrabold uppercase leading-[0.88] tracking-[-0.02em] text-white sm:text-display-lg lg:text-display-xl">{title}</h1>
