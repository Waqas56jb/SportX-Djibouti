...
function TimeBlock({ value, label }: { value: number; label: string }) {
  return (
    <div className="min-w-[60px] flex-1 border border-white/15 px-2 py-3 sm:flex-none sm:px-3 text-center sm:min-w-[76px]">
      ...
      <div className="container-site grid grid-cols-1 gap-12 py-16 sm:py-24 lg:grid-cols-[1fr_1.1fr] lg:items-center lg:gap-16">
        <div>
          <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-accent">
            <span className="h-2 w-2 animate-pulse rounded-full bg-accent" aria-hidden /> {t('home.flash.eyebrow')}
          </p>
          ...
          <p className="mt-5 max-w-md text-white/70">{t('home.flash.body')}</p>
          <div className="ltr-text mt-8 flex gap-2 sm:gap-3" role="timer" aria-live="off" aria-label={t('home.flash.timer', { d: time.days, h: time.hours, m: time.minutes })}>
            <TimeBlock value={time.days} label={t('home.flash.days')} />
            <TimeBlock value={time.hours} label={t('home.flash.hours')} />
            <TimeBlock value={time.minutes} label={t('home.flash.mins')} />
            <TimeBlock value={time.seconds} label={t('home.flash.secs')} />
          </div>
          ...
          {loading || !lead ? (
            ...
            <>
              <Link to={productPath(lead.slug)} className="group relative col-span-2 block aspect-[16/10] overflow-hidden bg-white/5">
                ...
                <span className="absolute start-4 top-4 bg-accent px-3 py-1.5 font-display text-xl font-bold text-ink">−{discountPercent(lead.price, lead.compareAtPrice)}%</span>
              ...
              {rest.slice(0, 2).map((p) => (
                <Link key={p.id} to={productPath(p.slug)} className="group relative block aspect-square overflow-hidden bg-white/5">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-ink/90 via-transparent to-transparent" />
                  <span className="absolute start-3 top-3 bg-accent px-2 py-1 text-xs font-bold text-ink">−{discountPercent(p.price, p.compareAtPrice)}%</span>
