...
export function SearchOverlay() {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[75]">
      ...
      <div ref={panel} role="dialog" aria-modal="true" aria-label="Search products" className="relative max-h-[100dvh] animate-slide-down overflow-y-auto bg-white shadow-lift">
        <div className="container-site">
          <form onSubmit={onSubmit} role="search" className="flex h-20 items-center gap-3 border-b border-ink sm:h-24">
            ...
            <input
              ...
              placeholder="Search boots, runners, jerseys…"
              autoComplete="off"
            ...
            />
            {loading && <Spinner className="h-5 w-5 text-ink-500" label="Searching" />}
            {term && (
            ...
            <button type="button" onClick={close} className="icon-btn" aria-label="Close search">
              <X className="h-6 w-6" />
            </button>
          </form>
              ...
              {recent.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-1">
                    {recent.map((r) => (
                      <li key={r} className="group flex items-center justify-between">
                        <button type="button" onClick={() => submit(r)} className="flex min-h-[40px] items-center gap-3 text-[15px] text-ink-700 hover:text-ink">
                          <Clock className="h-4 w-4 text-ink-500" aria-hidden /> {r}
                        </button>
                        <button type="button" onClick={() => forget(r)} className="p-2 text-ink-500 opacity-60 hover:text-ink group-hover:opacity-100" aria-label={`Remove ${r} from recent searches`}>
              ...
              {showResults ? (
                results.length > 0 ? (
                  <>
                    ...
                    <button type="button" onClick={() => submit(term)} className="group mt-8 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em]">
                      <span className="link-underline">View all results for “{query}”</span>
                  ...
                  !loading && (
                    <div className="py-6">
                      <p className="heading-md">No results for “{query}”</p>
                      <p className="mt-2 text-sm text-ink-500">Check the spelling or try a broader term like “boots”, “running” or “training”.</p>
                    </div>
                  )
                )
              ) : (
                <div className="hidden md:block">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {TRENDING.map((t) => (
                      <TrendingTile key={t.label} {...t} onNavigate={close} />
                    ))}
                  </div>
                </div>
              )}
...
const TRENDING: { label: string; to: string; img: ImageKey }[] = [
  { label: 'Football boots', to: '/football?category=football-boots', img: 'fbBootsOrange' },
  { label: 'Running shoes', to: '/running?category=running-shoes', img: 'runLace' },
  { label: 'Training apparel', to: '/training?category=tees', img: 'trCurl' },
];
...
function TrendingTile({ label, to, img, onNavigate }: { label: string; to: string; img: ImageKey; onNavigate: () => void }) {
  return (
    <Link to={to} onClick={onNavigate} className="group relative block aspect-[16/10] overflow-hidden">
      ...
      <span className="absolute bottom-3 left-3 font-display text-xl font-bold uppercase text-white">{label}</span>
    </Link>
  );
}
