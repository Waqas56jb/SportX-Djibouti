...
import { endOfDayIso, errorMessage, fromLocalInput, toLocalDateInput } from './utils';
...
export function CampaignFormDrawer({ open, campaign, onClose, onSaved, catalog }: { open: boolean; campaign?: Campaign; onClose: () => void; onSaved: (c: Campaign, created: boolean) => void; catalog: MarketingCatalog }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const saved = campaign ? await campaignService.updateCampaign(campaign.id, input) : await campaignService.createCampaign(input);
      toast.success(campaign ? 'Campaign updated.' : 'Campaign created.', { description: saved.name });
      onSaved(saved, !campaign);
    } catch (err) {
      toast.error('Couldn’t save campaign.', { description: errorMessage(err) });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <form id="campaign-form" onSubmit={submit} noValidate className="space-y-5">
        <FormGrid cols={2}>
          <Input label="Campaign name" required value={form.name} onChange={(e) => set('name', e.target.value)} error={errors.name} maxLength={60} placeholder="e.g. Derby Week" data-autofocus />
        ...
        <div>
          <div className="mb-1.5 flex items-baseline justify-between">
            ...
            {form.bannerUrl && (
              <button type="button" onClick={() => set('bannerUrl', undefined)} className="inline-flex items-center gap-1 text-xs font-medium text-red-600 hover:text-red-700">
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            ...
            <FileDropzone
              ...
              hint="1600 × 700 recommended · up to 4 MB"
              onFiles={([f]) => f && set('bannerUrl', URL.createObjectURL(f))}
        ...
        <FormGrid cols={2}>
          <MultiSelect label="Products" optional options={catalog.productOptions} value={form.productIds} onChange={(v) => set('productIds', v)} placeholder={catalog.loading ? 'Loading…' : 'Choose products'} maxChips={3} />
          <MultiSelect label="Categories" optional options={catalog.categoryOptions} value={form.categoryIds} onChange={(v) => set('categoryIds', v)} placeholder={catalog.loading ? 'Loading…' : 'Choose categories'} maxChips={3} />
